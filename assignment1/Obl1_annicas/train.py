...
def main(train_dir):
    ...
    def train_step(image, y):
        ...
        Args:
          image: float tensor of shape [batch_size, height, width, 3] rgb
          y: ground truth float tensor of shape [batch_size, height, width, 1] grayscale
        ...
        with tf.GradientTape() as tape:
            y_pred = model(image)
            loss = loss_fn(tf.cast(y, tf.float32), y_pred)
        ...
        #Update train loss variable with the loss (this may be done with the update state
    ...
    def val_step(image, y):
        ...
        #Update val loss variable with the calculated loss value.
        val_loss(loss)

        #Evaluate the predictions against the ground truth with the metric fn and update
